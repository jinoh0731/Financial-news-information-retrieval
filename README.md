...
- **Documents/Candidates**: In IR, documents/candidates can refer to any piece of textual information, such as web pages, news articles, scientific papers, books, emails, or any other kind of textual data. For our project, the documents refer to the various news articles that we wish to retrieve for the user within our large document repository.
...
The `all-MiniLM-L6-v2` is a model from the Sentence Transformers library, which is designed to produce state-of-the-art sentence embeddings. This model is a smaller, faster version of BERT and is optimized for generating embeddings that are particularly useful for tasks involving semantic similarity. The `all-mpnet-base-v2` model provides the best quality, while `all-MiniLM-L6-v2` is 5 times faster and still offers good quality.
...
Additionally, the Text-embedding-3 model supports up to 8191 tokens, significantly more than the 512 tokens limit of other tested models. Large context length capability is particularly important given our analysis which showed that a substantial proportion of our articles and queries exceeded the 512-token threshold, and were around 1200 tokens for each query and article text. The extended context length allowed by Text-embedding-3 ensures that more comprehensive information from longer articles is considered, enhancing the accuracy and relevance of the retrieved articles.

However, it's important to note that the `Text-embedding-3` model, while highly effective, requires resources including an OpenAI API key which incurs fees. For users with limited resources, the `Mini-LM` model presents a viable alternative. Although it processes a maximum of 512 tokens, strategies such as splitting longer articles into smaller segments or summarizing content can be employed to fit within this limit. These methods allow for the processing of extensive data without significant loss of context, ensuring that even with resource constraints, users can still achieve high-quality results in ESG news retrieval.
